...
There’s a ritual that’s become almost universal among developers adopting [AI coding agents](https://addyosmani.com/agentic-engineering/ai-coding-agent/). You set up a new repo, run `/init`, watch the agent scan your codebase, and get back a shiny AGENTS.md. It describes your directory structure, your tech stack, your testing conventions. You skim it, it looks reasonable, you commit it. You feel like you’ve done the responsible thing. Your agent is configured.
...
[The ACE framework](https://arxiv.org/abs/2510.04618) (Agentic Context Engineering, ICLR 2026) tackles this directly by treating context as an evolving playbook that adapts through a generator/reflector/curator pipeline, rather than a static file. On agent benchmarks, it outperformed static approaches by 12.3%. The architecture matters.
...
The most provocative finding in this space comes from [Arize AI’s prompt learning work](https://arize.com/blog/claude-md-best-practices-learned-from-optimizing-claude-code-with-prompt-learning/). Instead of manually writing CLAUDE.md instructions, they used an automated optimization loop - run the agent on training tasks, evaluate output, generate LLM feedback on why solutions failed, use meta-prompting to refine the instructions. Repeat.
...
If you are interested in video, Theo’s take on this topic is excellent too.
